 ...
 * 2) add middleware for checking if request have authorization tokens. look `validateToken` function
...
{
    const user = {id:1,username:'farhanahmed95'}
    const token = jwt.sign({user},'my_secret_key',{expiresIn:'30s'});
    res.json({token:token})
})
app.get("/api/user",validateToken,function(req,res)
{
